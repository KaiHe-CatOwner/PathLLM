...
class ScriptArguments:
    ...
    dataset_name_list: Optional[str] = field(default="CNX-PathLLM/PubMedPath,CNX-PathLLM/CleanedTextData,CNX-PathLLM/TwitterPath,CNX-PathLLM/Pathcap", metadata={"help": "CNX-PathLLM/CleanedTextData,CNX-PathLLM/TwitterPath,CNX-PathLLM/Pathcap"})
    ...
    eval_steps: Optional[int] = field(default=100000000, metadata={"help": "eval_steps"})
...
for dataset_name in script_args.dataset_name_list.split(","):
    dataset.append(load_dataset(dataset_name, split=split_text, cache_dir="/bask/projects/p/phwq4930-gbm/Zeyu/PathVLM/.cache"))
dataset = concatenate_datasets(dataset)
dataset = dataset.train_test_split(test_size=0.1)
dataset = dataset.rename_column('jpg', 'image')
dataset = dataset.map(formatting_func, num_proc=4, remove_columns=['txt','__key__', '__url__'])
