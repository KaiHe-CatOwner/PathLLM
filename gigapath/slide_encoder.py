...
class LongNetViT(nn.Module):
    ...
    def forward(self, querys ,x, coords, all_layer_embed=False):
        """
        ...
        if all_layer_embed:
            ...
            x_list = self.encoder2(src_tokens=None, token_embeddings=x_list)["encoder_out"]
            x_list = self.cross_attn2(querys,x_list,x_list)[0]
            x_list = self.norm(x_list)
            return x_list.to(torch.bfloat16)
